...
- Sentence shape: about **5.6 words** per sentence.
...
- Paragraph rhythm: about **0.0 words** per paragraph sample.
- CTA shape: about **2 words** per CTA.
- CTA verbs: `sign`, `watch`, `join`, `see`.
- Navigation label shape: about **1.9 words** per label.
- Lexical variety: **0.271** type-token ratio.
...
- Keep average sentence length between **4.2 and 7.0 words**.
- Keep headings near **2.7 words**; avoid generic one-word section labels unless the source uses them.
- Keep paragraph blocks near **0.0 words**.
- Keep CTAs near **2.0 words** and start them with: `sign`, `watch`, `join`, `see`.
...
- If writing a candidate file, run:
  `site2voice bench https://www.primevideo.com/ path/to/candidate.md --strict`
