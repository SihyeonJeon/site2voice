...
- Sentence shape: about **12.4 words** per sentence.
...
- Heading shape: about **12 words** per heading.
- Paragraph rhythm: about **15.8 words** per paragraph sample.
- CTA shape: about **2.4 words** per CTA.
- CTA verbs: `sign`, `try`, `download`, `get`, `start`, `explore`.
- Navigation label shape: about **2.1 words** per label.
- Lexical variety: **0.436** type-token ratio.
...
- Keep average sentence length between **9.3 and 15.5 words**.
- Keep headings near **12.0 words**; avoid generic one-word section labels unless the source uses them.
- Keep paragraph blocks near **15.8 words**.
- Keep CTAs near **2.4 words** and start them with: `sign`, `try`, `download`, `get`, `start`, `explore`.
...
- If writing a candidate file, run:
  `site2voice bench https://www.pinterest.com/about/ path/to/candidate.md --strict`
