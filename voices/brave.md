...
- Sentence shape: about **10.6 words** per sentence.
...
- Paragraph rhythm: about **14 words** per paragraph sample.
- CTA shape: about **4 words** per CTA.
- CTA verbs: `try`, `get`, `see`, `learn`.
- Navigation label shape: about **2.2 words** per label.
- Lexical variety: **0.276** type-token ratio.
...
- Keep average sentence length between **7.9 and 13.2 words**.
- Keep headings near **5.3 words**; avoid generic one-word section labels unless the source uses them.
- Keep paragraph blocks near **14.0 words**.
- Keep CTAs near **4.0 words** and start them with: `try`, `get`, `see`, `learn`.
...
- If writing a candidate file, run:
  `site2voice bench https://brave.com/ path/to/candidate.md --strict`
