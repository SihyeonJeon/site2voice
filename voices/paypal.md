...
- Sentence shape: about **6.5 words** per sentence.
...
- Heading shape: about **2.8 words** per heading.
- Paragraph rhythm: about **12.8 words** per paragraph sample.
- CTA shape: about **2.5 words** per CTA.
- CTA verbs: `buy`, `request`, `start`, `get`, `open`, `sign`, `contact`, `read`.
- Navigation label shape: about **2.6 words** per label.
- Lexical variety: **0.284** type-token ratio.
...
- Keep average sentence length between **4.9 and 8.1 words**.
- Keep headings near **2.8 words**; avoid generic one-word section labels unless the source uses them.
- Keep paragraph blocks near **12.8 words**.
- Keep CTAs near **2.5 words** and start them with: `buy`, `request`, `start`, `get`, `open`, `sign`, `contact`, `read`.
...
- If writing a candidate file, run:
  `site2voice bench https://www.paypal.com/us/business path/to/candidate.md --strict`
