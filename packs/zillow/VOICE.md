...
- Sentence shape: about **14.7 words** per sentence.
...
- Paragraph rhythm: about **45.2 words** per paragraph sample.
- CTA shape: about **2.8 words** per CTA.
- CTA verbs: `sign`, `join`, `buy`, `open`, `build`, `see`, `get`.
- Navigation label shape: about **2.1 words** per label.
- Lexical variety: **0.18** type-token ratio.
...
- Keep average sentence length between **11.0 and 18.4 words**.
- Keep headings near **5.3 words**; avoid generic one-word section labels unless the source uses them.
- Keep paragraph blocks near **45.2 words**.
- Keep CTAs near **2.8 words** and start them with: `sign`, `join`, `buy`, `open`, `build`, `see`, `get`.
...
- If writing a candidate file, run:
  `site2voice bench https://www.zillow.com/mortgages/ path/to/candidate.md --strict`
