...
- Sentence shape: about **14 words** per sentence.
...
- Heading shape: about **6.5 words** per heading.
- Paragraph rhythm: about **14.4 words** per paragraph sample.
- CTA shape: about **2.4 words** per CTA.
- CTA verbs: `sign`, `join`, `get`.
- Navigation label shape: about **1.8 words** per label.
- Lexical variety: **0.275** type-token ratio.
...
- Keep average sentence length between **10.5 and 17.5 words**.
- Keep headings near **6.5 words**; avoid generic one-word section labels unless the source uses them.
- Keep paragraph blocks near **14.4 words**.
- Keep CTAs near **2.4 words** and start them with: `sign`, `join`, `get`.
...
- If writing a candidate file, run:
  `site2voice bench https://www.linkedin.com/ path/to/candidate.md --strict`
