...
- Sentence shape: about **11.2 words** per sentence.
...
- Heading shape: about **3.8 words** per heading.
- Paragraph rhythm: about **11.6 words** per paragraph sample.
- CTA shape: about **1.7 words** per CTA.
- CTA verbs: `contact`, `explore`, `join`, `create`, `learn`.
- Navigation label shape: about **1.9 words** per label.
- Lexical variety: **0.336** type-token ratio.
...
- Keep average sentence length between **8.4 and 14.0 words**.
- Keep headings near **3.8 words**; avoid generic one-word section labels unless the source uses them.
- Keep paragraph blocks near **11.6 words**.
- Keep CTAs near **1.7 words** and start them with: `contact`, `explore`, `join`, `create`, `learn`.
...
- If writing a candidate file, run:
  `site2voice bench https://corp.roblox.com/ path/to/candidate.md --strict`
