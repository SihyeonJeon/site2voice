...
from .extract import clean_text, cta_score, words
...
def build_webfit_payload(voice_path: Path, site_path: Path, candidates: list[Path]) -> dict[str, Any]:
    voice_profile = json.loads(voice_path.read_text(encoding="utf-8"))
    site_profile = json.loads(site_path.read_text(encoding="utf-8"))
    return {
        "schema_version": "site2voice.webfit.v1",
        "reference": {
...
def webfit_json(payload: dict[str, Any]) -> str:
    return json.dumps(payload, ensure_ascii=False, indent=2) + "\n"


def webfit_markdown(payload: dict[str, Any]) -> str:
    lines = [
...
def webfit_failures(
    ...
    *,
    ...
    max_mimic_risk: float | None = None,
) -> list[str]:
    failures: list[str] = []
    candidates = payload["candidates"]
    by_label = {item["label"]: item for item in candidates}
    if min_delta is not None and {"without-context", "with-site-voice"} <= set(by_label):
        delta = by_label["with-site-voice"]["reference_fit"] - by_label["without-context"]["reference_fit"]
        if delta < min_delta:
            failures.append(f"reference-fit delta {delta:.1f} < {min_delta:.1f}")
    for item in candidates:
        label = item["label"]
        copy_safety = float(item["voice_fit"]["scores"]["copy_safety"])
        if min_copy_safety is not None and copy_safety < min_copy_safety:
            failures.append(f"{label}: copy safety {copy_safety:.1f} < {min_copy_safety:.1f}")
        mimic_risk = float(item["mimic_risk"])
        if max_mimic_risk is not None and mimic_risk > max_mimic_risk:
            failures.append(f"{label}: mimic risk {mimic_risk:.1f} > {max_mimic_risk:.1f}")
    return failures
