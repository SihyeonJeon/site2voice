...
the product's public voice.

`site2voice` sits in that gap:

```text
website copy -> VOICE.md + voice.json -> agent writes copy -> bench gate
```
...
- It avoids a weak claim: it does not "clone a brand"; it extracts measurable
  copy signals and checks drift.
